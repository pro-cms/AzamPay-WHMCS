...
foreach ($response as $key => $value) {
    // $log = $key . '=' . $value . '<br />\n\r';
    //file_put_contents('C:/xampp2/htdocs/whmcs/modules/gateways/azampay/modules/gateways/callback/log_' . date("yyyyMMddHHmmss") . '.log', $log, FILE_APPEND);
    if ($key == "transactionstatus") {
        ...
        $paymentAmount = $value;
    } else if ($key == "utilityref") {
        $invoiceId_ = $value;
    }
}

$transactionStatus = ($success == "success" ? 'Success' : 'Failure');
 ...
 */
$invoiceId = checkCbInvoiceID($invoiceId_, $gatewayParams['name']);
